 ...
 */
export function decodeOnlyFirst(buffer: Buffer): any {
  return decode(buffer, 1);
}

export function decode(buffer: any, expectedLength: number): any {
  return decodeCborStream(buffer, expectedLength);
}

function decodeCborStream(buffer: any, expectedLength: number): any {
  buffer = buffer.slice();
  ...
  for (let i = 0; i < buffer.length; i++) {
    const tlv = getTLVForNext(workbuffer);
    ...
    switch (tlv.TAG) {
      ...
      case 'BYTE_STRING':
        if (tlv.LEN === Infinity) {
          const length = findLegthOfIndefiniteLengthBuffer(tlv.VAL);
      ...
      case 'FLOAT_AND_NO_CONTENT':
        const type = float_and_no_content_semantics[tlv.VAL];
        switch (type) {
          case 'FALSE':
            results.push(false);
            i += 1;
...
const getTLVForNext = (buffer: any) => {
  ...
  const lennum = buffer[0] - (buffer[0] & 32) - (buffer[0] & 64) - (buffer[0] & 128);
  const tagnum: any = (buffer[0] - lennum) >> 5;
  let VAL;
  const TAG: any = tags[tagnum] as any;
...
const isEndianBig = () => {
  ...
  const u16 = new Uint16Array(buff);
...
const enforceBigEndian = (buffer: any) => {
  if (isEndianBig()) {
...
const findLegthOfIndefiniteLengthBuffer = (buffer: any) => {
  for (let i = 0; i < buffer.length; i++) {
    if (buffer[i] === 0xff) {
      return i + 1;
    }
...
const float_and_no_content_semantics: any = {
  20: 'FALSE',
  21: 'TRUE',
  22: 'NULL',
  23: 'UNDEFINED',
